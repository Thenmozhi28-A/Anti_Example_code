...
import { useUpdatePrescriptionMutation } from '../store/api/prescriptionsApi'
import toast from 'react-hot-toast'
import FormInput from './ui/FormInput'
import type { Prescription, EditPrescriptionModalProps } from '../types/TypesForAll'

export default function EditPrescriptionModal({ isOpen, onClose, prescription }: EditPrescriptionModalProps) {
    ...
    useEffect(() => {
        if (prescription) {
            setFormData({
                ...prescription,
                startDate: formatDateForInput(prescription.startDate),
                endDate: formatDateForInput(prescription.endDate)
            })
        }
    }, [prescription])
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault()
        if (!formData.patientName || !formData.patientId || !formData.medicineName || !formData.dosage || !prescription?.id) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
                <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    <div>
                        <h2 className="text-xl font-black text-slate-900 tracking-tight uppercase">Edit Prescription</h2>
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Update prescription details</p>
                    </div>
